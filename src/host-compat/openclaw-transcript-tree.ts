// Vendored verbatim from brodie's OpenClaw fork (brodie/master 32daff1cf1cf7e396a385328e2241cc8c7bd77f7,
// src/config/sessions/transcript-tree.ts). The host does not export its transcript tree through the
// plugin SDK, and the JSONL visible-projection shim must select the same active branch the host's own
// readers select. Keep this file identical to the fork module below this header apart from the
// `export` keywords on the two type aliases that the compat reader needs.
